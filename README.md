...
Website extraction prefers Chromium-rendered computed styles, captures a source screenshot, and falls back to bounded static HTML/CSS extraction. Rendered page-root colors and chromatic evidence take precedence over noisy stylesheet frequency so dark and light brands retain the correct semantic palette. Low-contrast overlay colors are rejected, logo discovery favors site icons and brand-specific assets, and component text falls back to a readable foreground when observed styles conflict. It rejects private-network targets and validates every HTTP redirect.
