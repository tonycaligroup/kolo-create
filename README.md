...
The browser also preserves rendered frames from large video, picture, CSS-background, and pseudo-element regions. These captures are classified separately as reference evidence or production media. Captures with embedded navigation, controls, or substantial webpage copy remain useful for brand judgment but cannot be placed in generated documents; the extractor continues looking for the underlying clean image URL. Automatic examples run in a dedicated brand-demonstration mode, allowing one signature production image even when the Kolo Create explainer does not share its subject. Normal documents still require semantic media relevance. If a media-led source yields only composite webpage captures, the example records a type-led fallback instead of cropping the webpage into a false hero image.
