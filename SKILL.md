...
```sh
uv run --project /home/node/.openclaw/workspace-main/skills/kolo-create kolo-create create design-system --url "<public website>" --name "<brand name>" --workspace "/home/node/.openclaw/kolo-create-data"
```
...
The create-design-system command always renders the bundled Kolo Create explainer to `<workspace>/examples/<brand-id>-kolo-create.pdf` and `<workspace>/examples/<brand-id>-kolo-create.pptx`. These automatic outputs run in brand-demonstration mode: they may use one signature production image even when the explainer's subject does not match the image, because their purpose is to demonstrate the newly captured brand. Normal user documents retain strict semantic media matching. Treat rendered webpage regions containing navigation, controls, or substantial embedded copy as reference evidence, never production media. Continue searching for the clean underlying image URL. If a media-, product-, or illustration-led source yields only reference-only composite captures, use and disclose a type-led fallback instead of placing or severely cropping the webpage capture. When extraction found logo assets, the automatic examples must use a valid one; otherwise disclose the text fallback. `--example-output` and `--example-presentation-output` are optional path overrides, not enable switches. The command returns both files in a top-level `artifacts` delivery manifest. Attach both to the current chat and require a delivery receipt for each before saying the design system is complete. Paths printed in prose are not delivery.

The design system includes more than tokens: capture observed heading levels, semantically ranked primary and secondary calls to action, component variants, cards, navigation, section surfaces, hero imagery, imagery proportions, borders, radii, shadows, padding, alignment, and common labels. Capture large rendered `<video>`, `<picture>`, CSS-background, and pseudo-element regions as bounded raster evidence in addition to direct image URLs; this preserves what the user sees for analysis when a video source or protected media endpoint cannot become a document image directly. Classify each capture deterministically. Embedded navigation, interactive controls, multiple text elements, or substantial visible copy make it reference-only; the bounded brand-director call may further restrict production use but must never override a deterministic rejection. Before sampling, dismiss common consent overlays. Prefer the visible header wordmark—including text-rendered brand links—over social-preview artwork, reject blank SVG-use rasterizations, reject browser-default or unpainted stylesheet colors as brand roles, and retain confidence plus provenance for major selections. When document roots are transparent, derive the canvas from dominant visible painted regions. Store a multi-color brand palette including a saturated dark support role when observed, portable font categories, overlay evidence, and visual-language signals such as media coverage, density, and whether the sampled viewport is product-, media-, illustration-, interface-, or typography-led. Return the separate component-inventory path as well.
...
```sh
uv run --project /home/node/.openclaw/workspace-main/skills/kolo-create kolo-create pdf create --system "<design-system.json>" --content "<source.txt-or-md>" --prompt "<design direction>" --output "<result.pdf>"
```
...
```sh
uv run --project /home/node/.openclaw/workspace-main/skills/kolo-create kolo-create pdf compare --system "<design-system.json>" --content "<source.txt-or-md>" --prompt "<design direction>" --output-dir "<comparison-folder>"
```
...
```sh
uv run --project /home/node/.openclaw/workspace-main/skills/kolo-create kolo-create pdf create --system "<design-system.json>" --content "<source.txt-or-md>" --prompt "<design direction>" --output "<result.pdf>" --planner llm --model "<workspace-entitled model>"
```
...
```sh
uv run --project /home/node/.openclaw/workspace-main/skills/kolo-create kolo-create powerpoint create --system "<design-system.json>" --content "<source.txt-or-md>" --prompt "<presentation direction>" --output "<result.pptx>"
```
...
The CLI's `delivery.status` begins as `pending-agent-delivery`. Do not translate this into success from the existence of a path. Send one attachment for every entry in the top-level `artifacts` list and require a returned message receipt before reporting the run complete.
